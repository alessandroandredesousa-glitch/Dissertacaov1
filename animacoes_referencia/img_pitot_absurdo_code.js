// IMAGEM ESTÁTICA — img_pitot_absurdo.png (Figura 4.13)
// Suposição do absurdo (volta de Pitot): supondo CD nao tangente a ω, a outra
// tangente por C encontra AD em D' (entre A e D). Forma-se o triangulo CDD',
// onde CD = CD' + D'D contraria CD < CD' + D'D. Mesma base da parte 2 da animacao.
// Destino: imagem/4.4.1/img_pitot_absurdo.png
...
// quadrilatero ABCD fechado (CD coral) + ω
...
// triangulo CDD' + tangente CD' (verde) + ponto de tangencia
drawPolygon({ points: [C, D, Dl], fill: true, fillColor: cor_azul_eletrico, opacity: 0.14 })
drawSegment({ points: [C, Dl], width: 0.03, color: cor_verde_neon })
drawPoint({ x: Ttop.x, y: Ttop.y, size: 0.055, color: cor_rosa_choque })
...
drawPoint({ x: Dl.x, y: Dl.y, size: 0.08, color: cor_verde_neon }); drawText({ text: "$D'$", x: Dl.x + 0.14, y: Dl.y - 0.34, fontSize: 0.42, color: cor_verde_neon })
