...
//drawGrid({ xMin: -5, xMax: 5, yMin: -8, yMax: 5 });
...
// TITULO
drawText({ text: "\\begin{center}\\textbf{Quadrilátero inscritível}\\end{center}", x: 0.2, y: 4.6, color: cor_titulo })
...
drawPoint({ x: A1.x, y: A1.y, size: 0.08, color: cor_dourado }); drawText({ text: "$A$", x: -2.39, y: 1.80, fontSize: 0.42, color: cor_dourado })
drawPoint({ x: B1.x, y: B1.y, size: 0.08, color: cor_dourado }); drawText({ text: "$B$", x: 0.30, y: -0.08, fontSize: 0.42, color: cor_dourado })
drawPoint({ x: C1.x, y: C1.y, size: 0.08, color: cor_dourado }); drawText({ text: "$C$", x: 2.08, y: 2.99, fontSize: 0.42, color: cor_dourado })
drawPoint({ x: D1.x, y: D1.y, size: 0.08, color: cor_dourado }); drawText({ text: "$D$", x: -0.89, y: 4.38, fontSize: 0.42, color: cor_dourado })
...
drawText({ text: "$\\hat{A} = \\frac{1}{2}\\,$arco$\\,BCD$", x: -4.7, y: yP1 + 1.2, fontSize: 0.36, color: cor_coral })
...
drawText({ text: "$\\hat{C} = \\frac{1}{2}\\,$arco$\\,BAD$", x: 2.1, y: yP1 + 1.2, fontSize: 0.36, color: cor_azul_eletrico })

pause()

// conclusao: os dois arcos completam 360° => Â+Ĉ = 180
drawText({ text: "arco$\\,BCD$ + arco$\\,BAD$ = $360^\\circ \\Rightarrow \\hat{A}+\\hat{C} = " + (angleDeg(A1, D1, B1) + angleDeg(C1, B1, D1)) + "^\\circ$", x: 0.2, y: yP1 - 2.4, fontSize: 0.36, color: cor_dourado })
...
drawText({ text: "\\begin{center}\\textbf{Volta: ângulos opostos suplementares}\\end{center}", x: 0.2, y: yP2 + 2.7, color: cor_titulo })

pause()

// Γ passa por A, B, C (4.2)
drawCircle({ center: O2, radius: R, fill: false, width: 0.03, color: cor_ciano })
drawText({ text: "$\\Gamma$", x: O2.x + R + 0.15, y: O2.y + R * 0.7, fontSize: 0.42, color: cor_ciano })
...
drawPoint({ x: A2.x, y: A2.y, size: 0.08, color: cor_dourado }); drawText({ text: "$A$", x: -2.39, y: -4.60, fontSize: 0.42, color: cor_dourado })
drawPoint({ x: B2.x, y: B2.y, size: 0.08, color: cor_dourado }); drawText({ text: "$B$", x: 0.30, y: -6.49, fontSize: 0.42, color: cor_dourado })
drawPoint({ x: C2.x, y: C2.y, size: 0.08, color: cor_dourado }); drawText({ text: "$C$", x: 2.08, y: -3.41, fontSize: 0.42, color: cor_dourado })
drawText({ text: "$\\Gamma$ passa por $A$, $B$, $C$ (4.2)", x: 0.2, y: yP2 - 2.6, fontSize: 0.34, color: cor_ciano })
...
drawText({ text: "$\\hat{B} = " + Bhat + "^\\circ$", x: B2.x - 0.4, y: B2.y - 0.6, fontSize: 0.4, color: cor_roxo_neon })
...
const desliza = param({ value: 0.5, min: 0, max: 1, step: 0.001, buttons: [{ value: 0.12, time: 2.5 }, { value: 0.88, time: 3 }, { value: 0.5, time: 2.5 }], label: "Deslizar X pelo arco (lado oposto a B)" })
animation(desliza, (s) => {
    const X = ptC(O2, gC + (0.12 + 0.76 * s) * (gA - gC))
    ...
    drawPoint({ x: X.x, y: X.y, size: 0.07, color: cor_dourado }); drawText({ text: "$X$", x: X.x - 0.1, y: X.y - 0.4, fontSize: 0.4, color: cor_dourado })
    drawText({ text: "$A\\hat{X}C = " + angleDeg(X, A2, C2) + "^\\circ = 180^\\circ-\\hat{B}$", x: -1.6, y: yP2 - 3.2, fontSize: 0.34, color: cor_laranja })
})

pause()

// D esta desse lado e D̂ = 180-B̂ => D no arco capaz => D em Γ
...
drawPoint({ x: D2.x, y: D2.y, size: 0.09, color: cor_verde_neon }); drawText({ text: "$D$", x: -0.89, y: -2.02, fontSize: 0.42, color: cor_verde_neon })
